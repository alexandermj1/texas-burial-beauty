...
export function buildQuoteReminderHtml(firstName: string, cemetery: string | null, amount: string | null, expiresOn: string, replyTo: string) {
  const mailto = `mailto:${replyTo}?subject=${encodeURIComponent(`Accepting our quote${cemetery ? ` — ${cemetery}` : ""}`)}`;
...
const buildPlain = (first: string, cemetery: string | null, amount: string | null, expiresOn: string) => [
  ...
  "If you would like to go ahead, simply reply to this email and we will prepare your agreement straight away.", "",
...
Deno.serve(async (req) => {
  ...
  if (previewEmail) {
    ...
    const raw = [`From: Texas Cemetery Brokers <${OUR_EMAIL}>`, `To: ${previewEmail}`, "Subject: Your quote expires in 3 days — Restland Memorial Park", "MIME-Version: 1.0", 'Content-Type: multipart/alternative; boundary="tcb-quote"', "", "--tcb-quote", 'Content-Type: text/plain; charset="UTF-8"', "", plain, "--tcb-quote", 'Content-Type: text/html; charset="UTF-8"', "", html, "--tcb-quote--"].join("\r\n");
  ...
  try {
    ...
    for (const sub of submissions ?? []) {
      ...
      const raw = [`From: Texas Cemetery Brokers <${OUR_EMAIL}>`, `To: ${email}`, `Subject: ${subject}`, ...replyHeaders, "MIME-Version: 1.0", 'Content-Type: multipart/alternative; boundary="tcb-quote"', "", "--tcb-quote", 'Content-Type: text/plain; charset="UTF-8"', "", plain, "--tcb-quote", 'Content-Type: text/html; charset="UTF-8"', "", html, "--tcb-quote--"].join("\r\n");
