// One-off Bayer Direct Purchase Offer — preview + send.
// Admin-only. Sends via Resend gateway from the Bayer brand identity.
import { corsHeaders } from "npm:@supabase/supabase-js@2/cors";
import { createClient } from "npm:@supabase/supabase-js@2";
import { renderBayerPurchaseOffer, PURCHASE_OFFER_DEFAULTS, type PurchaseOfferInput } from "../_shared/marketing/purchase-offer.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const input: PurchaseOfferInput = { ...PURCHASE_OFFER_DEFAULTS, ...(body.offer || {}) };
    const rendered = renderBayerPurchaseOffer(input);
