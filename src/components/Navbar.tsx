    ...
    >
      <div className="container mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6">
          <a href="tel:+12142304740" className={`inline-flex items-center gap-1.5 text-sm font-medium ${scrolled ? "text-primary" : "text-primary-foreground/80 hover:text-primary-foreground"}`}>
      ...
      {menuOpen && (
        <div className="md:hidden bg-background border-t border-border px-6 py-6 flex flex-col gap-4">
          <a href="tel:+12142304740" onClick={() => setMenuOpen(false)} className="inline-flex items-center gap-1.5 text-sm text-primary font-medium">
            <Phone className="w-3.5 h-3.5" />
            (214) 230-4740
          </a>
          {links.map(link => (
