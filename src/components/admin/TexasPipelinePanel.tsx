      ...
      <div className="bg-card rounded-xl border border-border/50 p-4 space-y-3">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {(submission as any).quote_response !== "accepted" && (
              <>
                <button
                  onClick={async () => {
                    const amt = Number((submission as any).accepted_quote_amount ?? (submission as any).quote_amount) || 0;
                    let patch: any = { quote_response: "accepted", quote_responded_at: new Date().toISOString() };
                    if (!amt) {
                      const entered = window.prompt("Accepted quote amount (USD) — total price seller accepted:");
                      const n = Number((entered || "").replace(/[^0-9.]/g, ""));
                      if (!n) return;
                      patch.accepted_quote_amount = n;
                      if (!(submission as any).quote_amount) patch.quote_amount = n;
                    }
                    await onPatch(patch);
                    toast({ title: "Quote accepted", description: "Moved to accepted — sales price tag now green." });
                  }}
