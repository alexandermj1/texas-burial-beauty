...
export function computeRequirements(
  ...
  if (hasFamilyTree(tree)) {
    // MASTER LOGIC — the only source of the document request.
    out.push(...masterRequirements(tree as V2State, rules, a.deed === "no"));
  } else {
    // No family tree on file yet: nothing can be derived, so the file is held
    // for review rather than guessed at by older question-by-question rules.
    add({
      code: "REVIEW",
      label: "Family tree not completed — document request cannot be built yet",
      why: "The document request is derived entirely from the completed family tree.",
      review: true,
    });
  }
